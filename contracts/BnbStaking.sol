...
contract BnbStaking is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accSpyPerShare; // Accumulated Spys per share, times 1e12. See below.
    }
    ...
    // The block number when SPY mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 spyReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accSpyPerShare = accSpyPerShare.add(spyReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accSpyPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 spyReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accSpyPerShare = pool.accSpyPerShare.add(spyReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit() public payable {
        ...
        updatePool(0);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accSpyPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accSpyPerShare).div(1e12);
